...
class Net(Module):
    ...
    def forward(self, x, hidden=None):
        ...
        Returns:
            linear_out, hidden: output and hidden state
        """        
...
def train(config, logger, train_and_valid_data):
    ...
    if config.add_train:
        ...
        logger.info("Load model from {}".format(config.model_save_path + config.model_name))
    ...
    for epoch in range(config.epoch):
        ...
        for i, _data in enumerate(train_loader):
            ...
            loss = criterion(pred_Y, _train_Y) + 0.3*torch.mean(torch.abs((pred_Y-_train_Y)/_train_Y))
        ...
        for _valid_X, _valid_Y in valid_loader:
            ...
            # calculate loss which is the weighted sum of MSE and MAPE
            loss = criterion(pred_Y, _valid_Y) + 0.3*torch.mean(torch.abs((pred_Y-_valid_Y)/_valid_Y))
