...
Returns: lose_truth, close, open_truth, open, high_truth, high, low_truth, low
"""
def draw(config, origin_data, logger, predict_norm_data: np.ndarray):
    ...
    for i in range(label_column_num):
        ...
        mape = np.mean(np.abs((label_data[config.predict_day:, i] - predict_data[:-
                       config.predict_day, i]) / label_data[config.predict_day:, i]))
        logger.info("The mean squared percentage error of stock {} is ".format(
            label_name[i]) + str(mape))
        print("The mean squared percentage error of stock {} is ".format(
            label_name[i]) + str(mape))
